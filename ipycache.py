...
def do_save(path, force=False, read=False):
    ...
    if force and read:
        raise ValueError(("The 'force' and 'read' options are " "mutually exclusive."))
...
def load_vars(path, vars):
    ...
    with open(path, "rb") as f:
        ...
        if missing_vars:
            raise ValueError(
                (
                    "The following variables could not be loaded "
                    "from the cache: {0:s}"
                ).format(", ".join(["'{0:s}'".format(var) for var in missing_vars]))
            )
        ...
        if additional_vars:
            raise ValueError(
                "The following variables were present in the cache, "
...
def cache(
    ...
    if do_save(path, force=force, read=read):
        ...
        try:
            ...
            vars_missing_str = ", ".join(["'{0:s}'".format(_) for _ in vars_missing])
            raise ValueError(
                (
                    "Variable(s) {0:s} could not be found in the "
                    "interactive namespace"
                ).format(vars_missing_str)
            )
        ...
        if verbose:
            print(
                "[Saved variables '{0:s}' to file '{1:s}'.]".format(
                    ", ".join(vars), path
                )
            )
    ...
    # variables from the specified file into the interactive namespace.
        ...
        if verbose:
            print(
                (
                    "[Skipped the cell's code and loaded variables {0:s} "
                    "from file '{1:s}'.]"
                ).format(", ".join(vars), path)
            )
...
class CacheMagics(Magics, Configurable):
    ...
    @magic_arguments.argument("vars", nargs="*", type=str, help="Variables to save.")
    @magic_arguments.argument(
        "-s",
        "--silent",
        action="store_true",
    ...
    @magic_arguments.argument(
        "-d", "--cachedir", help="Cache directory as an absolute or relative path."
    )
    @magic_arguments.argument(
        "-f",
        "--force",
    ...
    @magic_arguments.argument(
        ...
        default=False,
        help=(
            "Always read from the file and prevent the cell's execution, "
